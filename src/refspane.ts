...
import { columnDeclared } from "./tables.ts";
...
// every list that names a place. What declares the name says which column of a table it is in.
export async function openMention(app: App, path: string, line: number, declared: string) {
  await openAt(app, path, line, "", columnDeclared(declared));
}

// The two lists, drawn into `el`; shared by the pane and the section under a note, so a change to
// one shape changes both at once.
export function renderReferences(el: HTMLElement, refs: References, open: (path: string, line: number, declared: string) => void) {
  el.empty();
    ...
    for (const group of direction.groups) {
      ...
      for (const mention of group.mentions) {
        ...
        item.onClickEvent(() => {
          ...
          void open(mention.path, mention.at, mention.declared);
        });
      }
    }
  }
...
export class RefsPane extends ItemView {
  ...
  render(path: string | null, refs: References | null) {
    ...
    renderReferences(this.contentEl, refs, (p, line, declared) => void openMention(this.app, p, line, declared));
  }
}
