...
export function referencesIn(lines: string[], vocabulary: TypeVocabulary): Reference[] {
  ...
  for (let line = 1; line < end; line++) {
    ...
    if (item) {
      ...
      const ref = target ? span(line, text, item[1].length, target, optionalOf(f!.offer), key!, true) : null;
      if (ref) out.push(ref);
      continue;
    }
    ...
    const ref = target ? span(line, text, value![0].length, target, optionalOf(f!.offer), value![1], true) : null;
    if (ref) out.push(ref);
  }
  ...
  for (let line = Math.max(end + 1, 0); line < lines.length; line++) {
    ...
    if (text.startsWith("### ")) {
      ...
      const ref = target ? span(line, text, 4, target, optionalOf(grouped!), `## ${section}`) : null;
      if (ref) out.push(ref);
      continue;
    }
